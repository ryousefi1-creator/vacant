...
// Single source of truth for the mobile breakpoint — drives the layout switch
// (two-column desktop dashboard ↔ stacked, scrollable phone view).
...
export default function Live() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: isMobile ? 'auto' : '100vh', minHeight: '100dvh', width: '100%', overflow: isMobile ? 'visible' : 'hidden', color: '#0d1b2a',
      fontFamily: 'var(--font-geist-sans), system-ui, sans-serif',
      background: 'radial-gradient(900px 600px at 78% -8%, rgba(16,185,129,.16), transparent 60%), #eef3f1' }}>

      {/* header + tabs */}
      <header style={{ padding: isMobile ? '14px 16px 0' : '16px 22px 0', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
          <a href="/" style={{ fontWeight: 800, fontSize: isMobile ? 20 : 22, letterSpacing: '-.3px', display: 'flex', alignItems: 'center', gap: 10, color: 'inherit', textDecoration: 'none' }}>
          ...
          <div style={{ color: '#6b7a8d', fontSize: isMobile ? 11.5 : 12.5, fontWeight: 600, textAlign: 'right' }}>
            {lots.length} lot{lots.length !== 1 ? 's' : ''} · {streets.length} street{streets.length !== 1 ? 's' : ''} · {totalCars} live
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: '14px 0 12px', scrollbarWidth: 'thin', WebkitOverflowScrolling: 'touch',
          ...(isMobile ? { WebkitMaskImage: 'linear-gradient(90deg,#000 88%,transparent)', maskImage: 'linear-gradient(90deg,#000 88%,transparent)' } : {}) }}>
          ...
          {locs.map((l) => {
            ...
            return (
              <button key={l.id} onClick={() => setActive(l.id)} style={{
                ...
                borderRadius: 11, padding: isMobile ? '11px 14px' : '8px 13px', minHeight: isMobile ? 44 : undefined,
      ...
      {/* content — desktop: 2-col fixed-height; mobile: stacked + scrollable */}
      <div style={{
        flex: 1,
        ...
        <section style={{ position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center',
          padding: isMobile ? 14 : 18, height: isMobile ? '54vh' : undefined, minHeight: isMobile ? 300 : undefined, flexShrink: 0 }}>
          <div style={{ position: 'absolute', top: 14, left: isMobile ? 16 : 24, zIndex: 5, color: '#6b7a8d', fontSize: 12.5, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {isLot && (
            <div style={{ position: 'absolute', top: 12, right: isMobile ? 16 : 24, zIndex: 6, display: 'flex', gap: 4, background: '#fff', borderRadius: 10, padding: 3, boxShadow: '0 2px 8px rgba(13,27,42,.10)' }}>
        ...
        <aside style={{ background: '#fff',
          borderLeft: isMobile ? 'none' : '1px solid #e7ecf0',
          borderTop: isMobile ? '1px solid #e7ecf0' : 'none',
          padding: isMobile ? '20px 16px 36px' : '24px 24px',
          boxShadow: isMobile ? '0 -14px 30px rgba(13,27,42,.05)' : '-18px 0 40px rgba(13,27,42,.06)',
          overflowY: isMobile ? 'visible' : 'auto', scrollbarWidth: 'thin' }}>
          {isLot ? (
            <>
              <div style={{ color: '#6b7a8d', textTransform: 'uppercase', letterSpacing: '1.4px', fontSize: 11, fontWeight: 600 }}>
                Spaces open{estCap ? ' (est.)' : ''}
              </div>
              <div style={{ fontSize: isMobile ? 56 : 66, fontWeight: 800, lineHeight: .95, color: '#10b981', letterSpacing: '-2px', marginTop: 6 }}>
            ...
            <>
              ...
              <div style={{ fontSize: isMobile ? 56 : 66, fontWeight: 800, lineHeight: .95, color: '#3b82f6', letterSpacing: '-2px', marginTop: 6 }}>{carsHere}</div>
